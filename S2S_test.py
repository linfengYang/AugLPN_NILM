...
def load_dataset(filename, header=0):
    ...
    test_set_x = np.round(np.array(data_frame.iloc[:, 0], float), 5) # 5表示小数点的位数
...
test_set_x, test_set_y, ground_truth = load_dataset(loadname_test)  #这会加载全部测试数据集
...
test_provider = DoubleSourceProvider4(nofWindows=args.nosOfWindows, #预测窗口大小，默认为1000
                                      offset=offset,
